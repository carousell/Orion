...
func (d *DefaultServerImpl) buildHandlers() []*handlerInfo {
	...
	if !d.config.GRPCOnly {
		...
		if err != nil {
			logging.Error(context.Background(), "httpListener", "could not create listener", "error", err)
		}
		logging.Info(context.Background(), "HTTPListenerPort", httpPort)
		config := http.Config{
	...
	for sig := range c {
		if sig == syscall.SIGHUP { // only reload config for sighup
			if !d.config.HotReload {
				logging.Warn(context.Background(), "signal", "config reload SKIPPED (Hot reload disabled) on "+sig.String())
				continue
			}
			d.version++
			logging.Info(context.Background(), "signal", "config reloaded on "+sig.String())
			// relaod config
			err := readConfig(d.config.OrionServerName)
			if err != nil {
				notifier.NotifyWithLevel(err, "critical", "Error parsing config not reloading services")
				logging.Error(context.Background(), "Error", err, "msg", "not reloading services")
				continue
			}
			...
			for _, info := range oldServices {
				params := FactoryParams{
					ServiceName: info.sd.ServiceName,
					Version:     d.version - 1,
				}
...
func (d *DefaultServerImpl) registerService(sd *grpc.ServiceDesc, sf ServiceFactoryV2, reload bool) error {
	d.mu.Lock()
	defer d.mu.Unlock()
